<?php
namespace app\packages\XssHtml;
...
class XssHtml
{
     ...
     * PHP 富文本XSS过滤类
     * 使用方法：
     * use app\packages\XssHtml\XssHtml;
     * <1>默认规则
     * $html = '<html code>';//主要过滤的字符串
     * $xss = new XssHtml($html);//实例类
     * $html = $xss->getHtml();//获取过滤后的信息
     *
     * <2>自定义规则
     * ['strong', 'span'] 保留的标签
     * ['style'] 保留的属性
     * $html = '<html code>';//主要过滤的字符串
     * $xss = new XssHtml($html, 'utf-8', ['strong', 'span'], ['style']);//实例类
     * $html = $xss->getHtml();//获取过滤后的信息
     *
     * 注意 如果$html = '&lt;a&gt;';转义过的要用html_entity_decode()HTML实体转换为字符就会得到'<a>'
     */
    private $m_dom;
    private $m_xss;
    private $m_ok;
    private $m_AllowAttr = array('src', 'style');
    private $m_AllowTag = array( 'img', 'strong', 'span');
